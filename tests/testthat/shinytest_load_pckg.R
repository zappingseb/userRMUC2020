...
shinytest_load_pckg <- function(package_name = NULL) {
  ...
  if (dir.exists(pckg_dir)) {
    ...
    pckg_path_name <- list.files(pckg_dir, full.names = T)
    str <- paste0("\"", pckg_path_name, "\"")
    out <- parse(text = paste0("devtools::load_all(path = ", str, ", export_all = FALSE)"))
  } else if (file.exists(file.path(pth_source, "DESCRIPTION"))) {
    # for devtools::test, devtools::test_file, testthat::...
    out <- parse(text = paste0("devtools::load_all(\"", pth_source, "\", export_all = FALSE)"))
  } else {
    # for interactive() usage
    if (is.null(package_name)) {
      out <- parse(text = "library(devtools::as.package(\".\")$package, character.only = T)")
    } else {
      out <- parse(text = paste0("library(", package_name, ")"))
    }
  }
  return(out)
}
